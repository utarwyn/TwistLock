...
/**
 * Représente la fenêtre principale de jeu. Est le coeur de la partie IHM
 */
public class IHM extends JFrame
{
    ...
    public static final Color[] COULEURS = new Color[] { Color.RED , new Color( 39 , 174 , 96 ) , new Color( 241 , 196 , 15 ) , Color.BLUE };
     ...
     *
     * @param conteneur
     *         Conteneur concerné par la pose
     * @param coin
     *         Coin dans lequel poser le twistlock
     */
    public void choixTwistlock( Conteneur conteneur , int coin )
    {
        if( ! this.controleur.jouerTwistlock( conteneur , coin ) ) {
            ...
            JOptionPane.showMessageDialog( this , "Cet emplacement est déjà utilisé. Tu perds un twistlock de pénalité." , "Impossible de jouer ici" ,
        ...
        if( ! this.controleur.nouveauTour( ) ) {
            ...
            for( Joueur j : this.controleur.getJoueurs( ) ) {
                arrayList.add( j );
            }
            
            arrayList.sort( new Comparator< Joueur >( )
            {
                @Override
            ...
            for( int i = 0 ; i < arrayList.size( ) ; i++ ) {
                ...
                scores.append( i + 1 + " - " + joueur.getNom( ) ).append( ": " ).append( joueur.getScore( ) ).append( "\n" );
            }
            
            JOptionPane.showMessageDialog( this , scores.toString( ) , "FIN DE PARTIE - Tableau des scores" , JOptionPane.INFORMATION_MESSAGE );
        ...
        for( MainJoueur mainJoueur : this.mains )
            if( mainJoueur != null ) mainJoueur.miseAJour( this.controleur.getJoueurCourant( ) == mainJoueur.getJoueur( ) );
